-- Audit CRITICO #1 fix (MUY ALTA) -- create_project_with_sections() never
-- read or inserted is_roman, so every section was written with the
-- column's default (false) regardless of whether it was actually a
-- preliminary section (portada, dedicatoria, indice...) that should be
-- numbered i/ii/iii. The frontend now sends is_roman per section (see
-- hooks/useProject.ts, using fase.isRoman from TIPOS_TESIS) and this
-- redefines the function to read and store it, plus validates it like any
-- other section field. Same `create or replace function` pattern as
-- 0002_input_validation.sql -- safe to run whether or not that migration
-- has already been applied, and updates the function in place with no data
-- migration for existing rows (see note at the bottom for backfilling
-- projects created before this fix).

create or replace function public.create_project_with_sections(
  p_title text,
  p_tipo smallint,
  p_norma text,
  p_sections jsonb -- [{ "name": "...", "fase": "...", "order_index": 0, "is_roman": false }, ...]
)
returns public.projects
language plpgsql
security definer
set search_path = public
as $$
declare
  v_project public.projects;
  v_section_count int;
  v_section jsonb;
begin
  if auth.uid() is null then
    raise exception 'not authenticated';
  end if;

  if p_title is null or length(trim(p_title)) = 0 then
    raise exception 'title is required';
  end if;
  if length(p_title) > 300 then
    raise exception 'title too long (max 300 characters)';
  end if;

  if p_tipo is null or p_tipo not in (0, 1, 2) then
    raise exception 'invalid tipo: must be 0, 1 or 2';
  end if;

  if p_norma is null or p_norma not in ('libre', 'apa', 'vancouver') then
    raise exception 'invalid norma';
  end if;

  if p_sections is null or jsonb_typeof(p_sections) != 'array' then
    raise exception 'p_sections must be a JSON array';
  end if;
  v_section_count := jsonb_array_length(p_sections);
  if v_section_count = 0 then
    raise exception 'p_sections must not be empty';
  end if;
  if v_section_count > 200 then
    raise exception 'too many sections (max 200)';
  end if;

  for v_section in select * from jsonb_array_elements(p_sections)
  loop
    if v_section->>'name' is null or length(trim(v_section->>'name')) = 0 then
      raise exception 'every section requires a name';
    end if;
    if length(v_section->>'name') > 200 then
      raise exception 'section name too long (max 200 characters)';
    end if;
    if v_section->>'fase' is not null and length(v_section->>'fase') > 200 then
      raise exception 'section fase too long (max 200 characters)';
    end if;
    if (v_section->>'order_index')::int is null
       or (v_section->>'order_index')::int < 0
       or (v_section->>'order_index')::int > 500 then
      raise exception 'invalid order_index (must be between 0 and 500)';
    end if;
    -- is_roman is optional in the payload (defaults to false) so this
    -- function stays backwards compatible with any older caller that
    -- doesn't send it yet, but if present it must actually be a boolean.
    if v_section ? 'is_roman' and jsonb_typeof(v_section->'is_roman') != 'boolean' then
      raise exception 'is_roman must be a boolean';
    end if;
  end loop;

  insert into public.projects ("user", title, tipo, norma, word_count)
  values (auth.uid(), p_title, p_tipo, p_norma, 0)
  returning * into v_project;

  insert into public.sections (project, name, fase, order_index, is_roman, word_count)
  select
    v_project.id,
    (s->>'name')::text,
    (s->>'fase')::text,
    (s->>'order_index')::int,
    coalesce((s->>'is_roman')::boolean, false),
    0
  from jsonb_array_elements(p_sections) as s;

  return v_project;
end;
$$;

grant execute on function public.create_project_with_sections(text, smallint, text, jsonb) to authenticated;

-- Backfill for projects created before this fix: mark existing sections as
-- roman-numbered when their name matches an item that TIPOS_TESIS lists
-- under a roman-numbered fase (see frontend/src/types/index.ts). This
-- can't recover fase-level nuance for names that don't appear in the
-- current template (e.g. sections from a project type that has since
-- changed), but it correctly fixes the common preliminary sections for all
-- three thesis types.
update public.sections set is_roman = true
where is_roman = false
  and name in (
    'Portada', 'Dedicatoria', 'Agradecimientos', 'Resumen', 'Abstract',
    'Indice general', 'Indice de tablas', 'Indice de figuras',
    'Indice de tablas y figuras', 'Lista de abreviaturas',
    'Indice de cuadros comparativos'
  );
